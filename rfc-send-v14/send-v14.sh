...
#
# Send [PATCH v14 00/15] accel/rocket: RK3576 NPU (RKNN) enablement.
#
# WHAT v14 IS. v13 with what three maintainers and Sashiko asked for: Ulf
# Hansson's trims of 9 and 10, Philipp Zabel's single-reset API in 10, the
# reset pointer cleared under pmu->mutex on removal (Sashiko), Heiko
# Stuebner's split of v13's 13/14 into 13/15 and 14/15, his Reviewed-by on
# 7, and 3's comment and message updated for Igor Paunovic's re-runs on v13.
# The code differs from v13 in 3's comment and in 10 only. See
# rfc-send-v13/SENT.md, "What v14 owes".
#
# Recipients are v13's, which are scripts/get_maintainer.pl over the series
# plus the reviewers who are not in it.
#
set -euo pipefail
cd "$(dirname "$0")"
LOG=/tmp/v14-send.log
...
#
# v14-prep is next-20260914 with the fifteen patches rebuilt one by one from
# v13-prep-914: the two trees differ in 3's comment and in 10 only.
BRANCH=${BRANCH:-v14-prep}
#
# THE BASE STAYS next-20260914. It is v13's, the series was baselined on it
# again for v14 (rfc-send-v14/baseline.sh), and the cover says it applies to
# a plain next-20260914. If it has to move, this clone is SHALLOW:
#
#     git -C "$TREE" fetch --depth 1 origin tag next-YYYYMMDD
#     git -C "$TREE" rebase --onto next-YYYYMMDD next-20260914 v14-prep
#
# and then set BASE and re-run the baseline before sending.
#
...
if [ "$REGEN" = 1 ]; then
	...
	# this carefully.
	git -C "$TREE" format-patch --notes -v14 --cover-letter \
	    --base="$BASE" \
	    -o "$PWD" "$BASE..$BRANCH" >/dev/null
	for f in 0*.patch; do [ -e "$f" ] && mv "$f" "v14-$f"; done
	# THE COVER LETTER COMES BACK AS *** BLURB HERE ***. It is written in
	# cover-blurb.txt so that regenerating cannot throw it away.
	./splice-cover.py v14-0000-cover-letter.patch cover-blurb.txt
	...
	if grep -q '\*\*\*' v14-0000-cover-letter.patch; then
		echo "the cover still has a *** marker in it:" >&2
		grep -n '\*\*\*' v14-0000-cover-letter.patch >&2; exit 1
	fi
	...
	# nobody can see.
	b=$(grep -lc '^base-commit:' v14-*.patch 2>/dev/null | wc -l)
	[ "$b" -ge 1 ] || { echo "no base-commit trailer was emitted -- 5/13 cites one" >&2; exit 1; }
	echo "base-commit trailer on:"
	grep -l '^base-commit:' v14-*.patch | sed 's/^/  /'
	echo "regenerated $(ls v14-0*.patch | wc -l) patches"
	echo "01/15 is: $(sed -n 's/^Subject: //p' v14-0001-*.patch | head -1)"
	echo "         $(sed -n 's/^From: //p' v14-0001-*.patch | head -1)"
fi
...
CC=(--cc='royalnet026@gmail.com'          # Igor Paunovic, Tested-by 2,3,4/15, Reviewed-by 5/15
    #
    ...
    #
    --cc='abel.vesa@oss.qualcomm.com'     # Abel Vesa, Reviewed-by 9/15 and 10/15
    --cc='sebastian.reichel@collabora.com' # Sebastian Reichel, Reviewed-by 1/15
    --cc='sidong.yang@furiosa.ai'         # Sidong Yang, Tested-by 1/15
...
echo "Send $(sed -n 's/^Subject: //p' v14-0000-cover-letter.patch | head -1)"
echo "     From: $(sed -n 's/^From: //p' v14-0000-cover-letter.patch | head -1)"
printf '  To : %s\n' "${TO[@]#--to=}"
printf '  Cc : %s\n' "${CC[@]#--cc=}"
echo
ls v14-00*.patch | sed 's/^/  /'
echo
...
# that patch as 01/15, so there is no dependency outside the series left to
...
n=$(grep -lc '^Notes:' v14-*.patch 2>/dev/null | wc -l || true)
[ "$n" = 0 ] || { echo "a Notes block is back on $n patch(es); 01/15 is inside the series, so it has nothing left to say" >&2; exit 1; }
b=$(grep -lc '^base-commit:' v14-*.patch 2>/dev/null | wc -l || true)
...
q=$(grep -lc '^prerequisite-patch-id:' v14-*.patch 2>/dev/null | wc -l || true)
[ "$q" = 0 ] || { echo "prerequisite-patch-id is still on $q patch(es) -- the prerequisite did not get bundled" >&2; exit 1; }
# AND IGOR'S PATCH HAS TO BE 01/15 WITH HIS NAME ON IT. The whole point of
...
grep -q '^From: Igor Paunovic' v14-0001-*.patch || {
	echo "01/15 is not authored by Igor Paunovic" >&2; exit 1; }
for t in 'Sidong Yang' 'Diederik de Haas' 'Sebastian Reichel'; do
	grep -q "$t" v14-0001-*.patch || {
		echo "01/15 has lost $t's tag" >&2; exit 1; }
done
...
if [ "${DRY:-0}" = 1 ]; then
	...
	git send-email --dry-run --confirm=never "${TO[@]}" "${CC[@]}" v14-*.patch
	exit 0
fi

echo "This sends 16 messages for real (cover + 15). DRY=1 shows the headers instead."
read -rp "Send now? [y/N] " a; [[ "${a:-}" == [yY] ]] || { echo "aborted."; exit 1; }

git send-email --confirm=never "${TO[@]}" "${CC[@]}" v14-*.patch 2>&1 | tee "$LOG"
