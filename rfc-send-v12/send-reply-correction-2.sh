...
set -euo pipefail
cd "$(dirname "$0")"
M=reply-igor-correction-2.eml
[ -s "$M" ] || { echo "$M is missing or empty" >&2; exit 1; }

# ⚠ THE THREAD IS WHAT MAKES IT A CORRECTION RATHER THAN A NEW CLAIM.
grep -q '^In-Reply-To: <20260911122832\.1364839-1-gahing@gahingwoo\.com>' "$M" || {
	echo "$M does not reply to the mail it withdraws" >&2; exit 1; }
...
for s in 'They are yours' 'uniformly 128' 'two different' \
         'I withdraw it'; do
...
# what was withdrawn.
for s in '102 induced resets' '25 August'; do
...
"${CMD[@]}" 2>&1 | tee /tmp/reply-correction2-send.log
