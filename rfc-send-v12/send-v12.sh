...
#
# Send [PATCH v12 00/14] accel/rocket: RK3576 NPU (RKNN) enablement.
#
...
if [ "$REGEN" = 1 ]; then
	...
	./splice-cover.py v12-0000-cover-letter.patch cover-blurb.txt
	...
	# ⚠⚠ ANY *** MARKER, not only format-patch's two. v12's blurb carries
	# "*** BOARD RESULT HERE ***" where the answer to Sashiko's 4/14
	# finding goes, and it is there so that this cannot be sent before the
	# board has spoken. A cover with a marker in it is a draft.
	if grep -q '\*\*\*' v12-0000-cover-letter.patch; then
		echo "the cover still has a *** marker in it:" >&2
		grep -n '\*\*\*' v12-0000-cover-letter.patch >&2; exit 1
	fi
	...
	# nobody can see.
	b=$(grep -lc '^base-commit:' v12-*.patch 2>/dev/null | wc -l)
	[ "$b" -ge 1 ] || { echo "no base-commit trailer was emitted -- 5/13 cites one" >&2; exit 1; }
	echo "base-commit trailer on:"
	grep -l '^base-commit:' v12-*.patch | sed 's/^/  /'
	echo "regenerated $(ls v12-0*.patch | wc -l) patches"
	echo "01/14 is: $(sed -n 's/^Subject: //p' v12-0001-*.patch | head -1)"
	echo "         $(sed -n 's/^From: //p' v12-0001-*.patch | head -1)"
fi
...
echo "Send $(sed -n 's/^Subject: //p' v12-0000-cover-letter.patch | head -1)"
echo "     From: $(sed -n 's/^From: //p' v12-0000-cover-letter.patch | head -1)"
printf '  To : %s\n' "${TO[@]#--to=}"
printf '  Cc : %s\n' "${CC[@]#--cc=}"
echo
ls v12-00*.patch | sed 's/^/  /'
echo
...
q=$(grep -lc '^prerequisite-patch-id:' v12-*.patch 2>/dev/null | wc -l || true)
...
if [ "${DRY:-0}" = 1 ]; then
	...
	git send-email --dry-run --confirm=never "${TO[@]}" "${CC[@]}" v12-*.patch
	exit 0
fi
...
git send-email --confirm=never "${TO[@]}" "${CC[@]}" v12-*.patch 2>&1 | tee "$LOG"
