#!/usr/bin/env bash
# Send [RFC PATCH v5 0/8] accel/rocket: RK3576 NPU (RKNN) enablement.
...
echo "Send [RFC PATCH v5 0/8], From: Jiaxing Hu <gahing@gahingwoo.com>"
printf '  To : %s\n' "${TO[@]#--to=}"
printf '  Cc : %s\n' "${CC[@]#--cc=}"
...
git send-email --confirm=never "${TO[@]}" "${CC[@]}" \
  v5-0000-cover-letter.patch v5-0001-*.patch v5-0002-*.patch v5-0003-*.patch \
  v5-0004-*.patch v5-0005-*.patch v5-0006-*.patch v5-0007-*.patch v5-0008-*.patch \
  2>&1 | tee "$LOG"

CID=$(grep -m1 -oiE "Message-Id: <[^>]+>" "$LOG" | sed 's/.*<//;s/>.*//')
if [[ -n "$CID" ]]; then
  echo "$CID" > /tmp/v5-cover-msgid.txt
  echo
  echo "v5 cover Message-Id: $CID"
