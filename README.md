...
NPU probe verified on hardware (2026-06-08):
...
4-patch series in `kernel/`, against linux-next-20260527:

```
0001  dt-bindings: npu: rockchip,rk3588-rknn-core: add rk3576
0002  arm64: dts: rockchip: rk3576: add RKNN NPU subsystem
0003  arm64: dts: rockchip: rk3576-rock-4d: enable NPU core 0
0004  accel: rocket: add RK3576 support
```
...
```
build.sh                     full pipeline (extract → mesa → buildroot → sdcard.img)
kernel-only.sh               fast kernel iteration
kernel/
  000[1-4]-*.patch           4-patch series for upstream submission
