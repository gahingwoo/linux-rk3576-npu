...
# streams()/decode() come from charsiu's rkllm_regcmd.py rather than a second
# parser written here, so the container format has one implementation.
for _p in ("/home/parallels/Desktop/charsiu/tools",
           os.path.expanduser("~/Desktop/charsiu/tools"),
           os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "..", "charsiu", "tools")):
    if os.path.isfile(os.path.join(_p, "rkllm_regcmd.py")):
        sys.path.insert(0, _p)
        break
from rkllm_regcmd import streams, decode                      # noqa: E402

GEOM = (sys.argv[1] if len(sys.argv) > 1 else
        "/home/parallels/Desktop/linux-rk3576-npu/vendor-capture/geom")
...
def main():
    ...
    p("And a per-model constant cannot be right, because %d files hold more" % len(mixed))
    p("than one value inside a single compile:")
    p()
    def dump(n):
        f = per_file[n]
